...
export function parseServeArgs(args: readonly string[]): ServeArgs {
  try {
    ...
    if (Number.isNaN(idle)) {
      throw new ServeArgsError(`invalid idle value: "${idleStr}" is not a number`)
    }

    if (idle <= 0) {
      throw new ServeArgsError(`invalid idle value: must be positive (got ${idle})`)
    }

    if (!Number.isInteger(idle)) {
      throw new ServeArgsError(`invalid idle value: must be an integer (got ${idle})`)
    }
 ...
 */
export async function cmdServe(args: readonly string[], flags: OutputOptions): Promise<number> {
  // Parse arguments
  ...
  try {
    ...
    error(`failed to start server: ${err instanceof Error ? err.message : String(err)}`, flags)
    await picker.close()
    return 1
  }
