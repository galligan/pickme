...
describe("escapeFTSQuery", () => {
  ...
  test("handles mixed separators", () => {
    expect(escapeFTSQuery("src/components button")).toBe(
...
describe("upsertFile", () => {
  ...
  test("inserts new file", () => {
    ...
    const result = db
      .query<{ path: string }, []>("SELECT path FROM files_meta")
      .get();
  ...
  test("updates existing file", () => {
    ...
    const result = db
      .query<{ mtime: number; count: number }, []>(
...
describe("upsertFiles", () => {
  ...
  test("handles empty array", () => {
    ...
    const result = db
      .query<{ count: number }, []>("SELECT COUNT(*) as count FROM files_meta")
      .get();
...
describe("upsertFrecency", () => {
  ...
  test("updates existing frecency records", () => {
    ...
    const result = db
      .query<{ git_recency: number; git_frequency: number }, []>(
...
describe("pruneDeletedFiles", () => {
  ...
  test("handles large existing set efficiently", () => {
    ...
    const existingPaths = new Set(
      Array.from({ length: 50 }, (_, i) => `/test/file${i}.ts`)
    );
...
describe("error handling", () => {
  test("throws DatabaseError on connection failure", () => {
    ...
    expect(() => openDatabase("/nonexistent/deeply/nested/path/db.sqlite")).toThrow(
      DatabaseError
    );
  });
